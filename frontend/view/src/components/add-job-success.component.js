...
export default class AddJobSuccess extends Component {  
    constructor(props) {
    super(props);
    this.state = {
        data: [],
    ...
    componentDidMount() {
        ...
        if (user) {
            this.setState({
                currentUser: user,
                showManager: user.roles.includes("ROLE_MANAGER") || user.roles.includes("ROLE_ADMIN"),
                ...
                <div className = "test">
                    <div className="formblock" align = 'center'>
                        <ReactBootstrap.Table striped bordered hover>
                            ...
                            {!showManager && ( <tbody>
                            {this.state.data.map(this.renderJobForUser)}
                            </tbody>)}
                            {showManager && ( <tbody>
                            {this.state.data.map(this.renderJobForManager)}
                            </tbody>)}
